...
docker run -d --name db1 --network webserver1-network -e MARIADB_ROOT_PASSWORD=michel -e DB_INSTANCE=db1 -v db1-data:/var/lib/mysql mariadb
docker run -d --name db2 --network webserver2-network -e MARIADB_ROOT_PASSWORD=michel -e DB_INSTANCE=db2 -v db2-data:/var/lib/mysql mariadb
docker run -d --name db3 --network webserver3-network -e MARIADB_ROOT_PASSWORD=michel -e DB_INSTANCE=db3 -v db3-data:/var/lib/mysql mariadb
